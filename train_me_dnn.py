...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    save_dir, epochs, batch_size, weights, single_cls, data, resume, noval, nosave, workers, supp_weights, train_yolo, weights_me= \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.data, \
        opt.resume, opt.noval, opt.nosave, opt.workers, opt.supp_weights, opt.train_yolo, opt.weights_me
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (ref1, ref2, target) in pbar:  # batch -------------------------------------------------------------
            ...
            ref2 = ref2.to(device, non_blocking=True).float() / 255
            target = target.to(device, non_blocking=True).float() / 255
