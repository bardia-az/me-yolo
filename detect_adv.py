    ...
    for path, img, im0s, vid_cap, s in dataset:
        ...
        if(autoencoder is not None):
            ...
            if visualize:
                ...
                rec_imgs *= 255
                rec_imgs = torch.clamp(rec_imgs, 0, 255)
                rec_imgs = torch.round(rec_imgs)
                cv2.imwrite(str(visualize / 'reconstructed.png'), rec_imgs[0].cpu().numpy().astype(np.uint8).transpose((1,2,0))[:,:,::-1])
        else:
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default=ROOT / 'data/rec_images', help='directory containing the test images')
    ...
    parser.add_argument('--project', default=ROOT / 'runs/detect_adv', help='save results to project/name')
